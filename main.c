...
int main(void) {
  ...
  while (dots_spawned < MAX_DOTS) {
    ...
    if (levelData[ay * LEVEL_W + ax].r == 255 && dist > LEVEL_W / 8) {
      ...
      dots[dots_spawned].last_pos =
          (Vector2){dots[dots_spawned].rect.x, dots[dots_spawned].rect.y};
  ...
  while (!WindowShouldClose()) {
    ...
    for (int y = 0; y < LEVEL_H; y++) {
      for (int x = 0; x < LEVEL_W; x++) {
        if (levelData[y * LEVEL_W + x].r == 0 &&
            levelData[y * LEVEL_W + x].g == 0 &&
            levelData[y * LEVEL_W + x].b == 0) {
          Rectangle wall = {x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE, TILE_SIZE};
          if (CheckCollisionRecs(nextPos, wall))
            collided = true;
        }
      }
    }
      ...
      for (int y = 0; y < LEVEL_H; y++) {
        for (int x = 0; x < LEVEL_W; x++) {
          if (levelData[y * LEVEL_W + x].r == 0 &&
              levelData[y * LEVEL_W + x].g == 0 &&
              levelData[y * LEVEL_W + x].b == 0) {
            Rectangle wall = {x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE,
                              TILE_SIZE};
            if (CheckCollisionRecs(nextPos, wall))
              collided = true;
          }
      ...
      for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
          ...
          for (int y = 0; y < LEVEL_H && !blocked; y++) {
            for (int x = 0; x < LEVEL_W && !blocked; x++) {
              if (levelData[y * LEVEL_W + x].r == 0) {
                Rectangle wall = {x * TILE_SIZE, y * TILE_SIZE, TILE_SIZE,
                                  TILE_SIZE};
                if (CheckCollisionRecs(test, wall))
                  blocked = 1;
              }
            }
          }
          ...
          valid_moves[valid_count++] =
              (Vector2){dx * DOT_SPEED, dy * DOT_SPEED};
          // Score: maximize distance from player, minimize proximity to other
          // dots
          ...
          for (int j = 0; j < MAX_DOTS; j++) {
            if (i == j || !dots[j].alive)
              continue;
            float d =
                sqrtf((test.x - dots[j].rect.x) * (test.x - dots[j].rect.x) +
                      (test.y - dots[j].rect.y) * (test.y - dots[j].rect.y));
            if (d < min_dist_dot)
              min_dist_dot = d;
          }
          float score =
              dist_player + 0.7f * min_dist_dot; // weight for spreading
          if (score > best_score) {
            best_score = score;
    ...
    for (int i = 0; i < MAX_DOTS; i++) {
      if (dots[i].alive) {
        DrawCircle((int)(dots[i].rect.x + dots[i].rect.width / 2),
                   (int)(dots[i].rect.y + dots[i].rect.height / 2),
                   dots[i].rect.width / 2, (Color){139, 69, 19, 255}); // brown
      }
    }
    EndMode2D();
